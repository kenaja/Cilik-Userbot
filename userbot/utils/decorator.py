    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            grey_reg = sudo_reg = re.compile(pattern)
        else:
            grey_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
            grey_reg = re.compile(grey_ + pattern)
            ...
            if command is not None:
                ...
                cmd1 = (
                    (grey_ + pattern).replace("$", "").replace("\\", "").replace("^", "")
                )
                cmd2 = (
                    (sudo_ + pattern)
                    .replace("$", "")
    ...
    def decorator(func):
        if not disable_edited:
            bot.add_event_handler(
                func, events.MessageEdited(**args, outgoing=True, pattern=grey_reg)
            )
        bot.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=grey_reg)
        )
        if allow_sudo:
            if not disable_edited:
                bot.add_event_handler(
