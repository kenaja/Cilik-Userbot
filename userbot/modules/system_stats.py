...
import psutil
from userbot import ALIVE_LOGO, ALIVE_NAME, BOT_VER, CMD_HELP, CILIK_TEKS_KUSTOM, StartTime, UPSTREAM_REPO_BRANCH, bot
...
@register(outgoing=True, pattern=r"^\.(?:cilikon)\s?(.)?")
async def amireallyalive(alive):
    await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
        f"●▬▬▬▬▬▬ஜ۩۞۩ஜ▬▬▬▬▬▬● \n"
        f"✾ 🤴 • `ᴏᴡɴᴇʀ    :`[Grey](t.me/greyyvbss) \n"
